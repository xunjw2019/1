...
# 读取CSV文件
df = pd.read_csv(r'C:\Users\11943\PycharmProjects\pythonProject3\data\changchun.csv')
...
# 绘制降雪量(SNWD)的柱状图
plt.figure(figsize=(12, 6))
sns.barplot(x='Month', y='SNWD', hue='Season', data=df, ci=None)
plt.title('Monthly Snow Depth by Season')
plt.xlabel('Month')
...
# 绘制温度和降雪关系的散点图
plt.figure(figsize=(10, 6))
sns.scatterplot(x='TAVG', y='SNWD', data=df, hue='Season')
...
# 数据分析：基本统计信息
summary_stats = df[['PRCP', 'SNWD', 'TAVG', 'TMAX', 'TMIN']].describe()
...
# 数据分析：相关性矩阵
correlation_matrix = df[['PRCP', 'SNWD', 'TAVG', 'TMAX', 'TMIN']].corr()
